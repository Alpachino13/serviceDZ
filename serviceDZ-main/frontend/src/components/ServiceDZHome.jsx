...
function ExpertCard({ expert, index }) {
  ...
  return (
    <motion.article
      ...
      onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
      aria-label={`${expert.name}, ${expert.specialty}`}
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
          <span style={{ fontSize: 11, fontWeight: 700, color: "#EF9F27", background: "#EF9F2715", border: "1px solid #EF9F2730", borderRadius: 6, padding: "2px 7px" }}>
            {expert.rating === "Nouveau" ? "Nouveau" : `★ ${expert.rating}`}
          </span>
          ...
          <AnimatePresence>
            {query.length > 0 && (
              <motion.button
                ...
                onClick={() => setQuery("")} aria-label="Effacer"
...
export default function ServiceDZHome() {
  ...
  // La nouvelle fonction de recherche qui interroge ta DB
  const handleSearch = async (query) => {
    ...
    try {
      const res = await fetch(`${API}/api/users/workers`, {
        headers: { "ngrok-skip-browser-warning": "true" }
      });
      const dbWorkers = await res.json();

      // Filtrer les résultats (nom, ville, spécialité)
      ...
      // Formater avec "Nouveau" et 0 jobs
      const formatted = filtered.map((w, idx) => {
        const catColor = CATEGORIES.find(c => w.specialty?.includes(c.label))?.color || "#185FA5";
        return {
          id: w._id || idx,
          name: w.name,
      ...
      // Fait défiler la page vers les résultats
      setTimeout(() => {
      ...
      console.error(err);
      setSearchError("Impossible de récupérer les artisans.");
      setResults([]);
    } finally {
    ...
    <div style={{ minHeight: "100vh", background: C.walnut, color: C.text, fontFamily: "'DM Sans', 'Segoe UI', sans-serif", overflowX: "hidden" }}>
      ...
      <div style={{ position: "relative", zIndex: 1 }}>
            ...
            {isLoading ? (
              ...
              <>
                ...
                <button onClick={() => logout({ logoutParams: { returnTo: "/" } })}
                  style={{ all: "unset", cursor: "pointer", fontSize: 13, color: C.muted, padding: "8px 16px", border: `1px solid ${C.border}`, borderRadius: 9 }}
                >Déconnexion</button>
              </>
            ) : (
              <>
                <button onClick={() => navigate("/login")} style={{ all: "unset", cursor: "pointer", fontSize: 13, color: C.textMuted, padding: "8px 16px", border: `1px solid ${C.border}`, borderRadius: 9 }}>Connexion</button>
                <button onClick={() => navigate("/login")} style={{ all: "unset", cursor: "pointer", fontSize: 13, fontWeight: 600, color: "#fff", padding: "8px 16px", background: C.electric, borderRadius: 9 }}>Inscription</button>
              </>
            )}
          </div>
        </motion.nav>
        ...
        <header style={{ textAlign: "center", padding: "72px 40px 60px" }}>
          ...
          >Trouvez le bon <span style={{ color: "transparent", backgroundClip: "text", WebkitBackgroundClip: "text", backgroundImage: `linear-gradient(135deg, ${C.electric}, ${C.electricLt})` }}>réparateur</span><br />près de chez vous</motion.h1>
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.25 }}
            style={{ fontSize: 16, color: C.muted, margin: "0 0 40px", lineHeight: 1.7, maxWidth: 460, marginInline: "auto" }}
          >Des artisans vérifiés, des tarifs transparents, une intervention rapide en Algérie.</motion.p>
          ...
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }} style={{ marginTop: 16, fontSize: 12, color: C.muted }}>
            Suggestions : {["Plomberie", "Électricité", "Peinture"].map((s, i) => (
              <button key={s} onClick={() => handleSearch(s)}
                style={{ all: "unset", cursor: "pointer", color: C.electricLt, marginLeft: i > 0 ? 12 : 6, fontSize: 12, textDecoration: "underline" }}
              >{s}</button>
            ))}
          </motion.p>
        </header>
        ...
        <AnimatePresence>
          {searchQuery && (
            <motion.section id="search-results-section" key="search-results"
            ...
            >
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
                <h2 style={{ margin: 0, fontSize: 18, fontWeight: 700, color: C.text }}>Résultats pour <span style={{ color: C.electric }}>« {searchQuery} »</span></h2>
                <button onClick={clearSearch} style={{ all: "unset", cursor: "pointer", fontSize: 12, color: C.muted, textDecoration: "underline" }}>Effacer</button>
              </div>

              {searchError && <p style={{ color: "#E24B4A", fontSize: 13, marginBottom: 16 }}>⚠ {searchError}</p>}

              <div style={{ display: "grid", gap: 12, gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))" }}>
                {isSearching
                  ? [0, 1, 2].map(i => <SkeletonCard key={i} />)
                  : results && results.length > 0
                    ? results.map((expert, i) => <ExpertCard key={expert.id} expert={expert} index={i} />)
                    : !isSearching && <p style={{ color: C.muted, fontSize: 14, gridColumn: "1/-1" }}>Aucun artisan trouvé pour cette recherche.</p>
                }
              </div>
        ...
        <main style={{ maxWidth: 1120, margin: "0 auto", padding: "0 40px 80px" }}>
          <section style={{ marginBottom: 64 }}>
            <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", marginBottom: 28 }}>
              <h2 style={{ margin: 0, fontSize: 20, fontWeight: 700, letterSpacing: "-0.02em" }}>Catégories de services</h2>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(140px, 1fr))", gap: 12 }}>
              {CATEGORIES.map((cat, i) => (
                <div key={cat.id} onClick={() => handleSearch(cat.label)} style={{ cursor: "pointer" }}>
          ...
          <section>
            <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", marginBottom: 28 }}>
              <h2 style={{ margin: 0, fontSize: 20, fontWeight: 700, letterSpacing: "-0.02em" }}>Réparateurs recommandés</h2>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))", gap: 12 }}>
              {pageLoading
                ? [0, 1, 2].map(i => <div key={i}><SkeletonCard /></div>)
                : FEATURED.map((expert, i) => <div key={expert.id}><ExpertCard expert={expert} index={i} /></div>)
              }
            </div>
          </section>
        </main>
