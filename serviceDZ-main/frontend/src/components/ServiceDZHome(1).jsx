...
const CATEGORIES_META = [
  { icon: "🔧", label: "Plomberie",     color: "#378ADD" },
  { icon: "⚡", label: "Électricité",   color: "#EF9F27" },
  { icon: "🧱", label: "Maçonnerie",    color: "#D85A30" },
  { icon: "🪟", label: "Menuiserie",    color: "#1D9E75" },
  { icon: "❄️", label: "Climatisation", color: "#5DCAA5" },
  { icon: "🎨", label: "Peinture",      color: "#D4537E" },
  { icon: "🔩", label: "Serrurerie",    color: "#888780" },
  { icon: "🏠", label: "Rénovation",    color: "#AFA9EC" },
];
...
export default function ServiceDZHome() {
  ...
  const [categories, setCategories]   = useState(CATEGORIES_META.map(c => ({ ...c, count: 0 })));

  // Fetch vrais comptages par spécialité depuis l'API
  useEffect(() => {
    const API = (process.env.REACT_APP_API_URL || "").replace(/\/$/, "");
    fetch(`${API}/api/users/workers`, { headers: { "ngrok-skip-browser-warning": "true" } })
      .then(r => r.ok ? r.json() : [])
      .then(workers => {
        if (!Array.isArray(workers)) return;
        setCategories(CATEGORIES_META.map(cat => ({
          ...cat,
          count: workers.filter(w =>
            (w.specialty || "").toLowerCase().includes(cat.label.toLowerCase())
          ).length,
        })));
      })
      .catch(() => {})
